...
var i,
	...
	***REMOVED***,
	...
	runescape = new RegExp( "\\\\([\\da-f]{1,6***REMOVED***" + whitespace + "?|(" + whitespace + ")|.)", "ig" ),
	...
	***REMOVED***
	...
	if ( documentIsHTML && !seed ) {
		...
		if ( (match = rquickExpr.exec( selector )) ) {
			...
			***REMOVED*** else if ( (m = match[3]) && support.getElementsByClassName && context.getElementsByClassName ) {
	...
	if ( parent && parent !== parent.top ) {
		...
		if ( parent.addEventListener ) {
			parent.addEventListener( "unload", function() {
				setDocument();
			***REMOVED***, false );
		***REMOVED*** else if ( parent.attachEvent ) {
					...
					if ( parent ) {
						...
						***REMOVED***
						...
						if ( forward && useCache ) {
							...
							outerCache = parent[ expando ] || (parent[ expando ] = {***REMOVED***);
						...
						***REMOVED*** else if ( useCache && (cache = (elem[ expando ] || (elem[ expando ] = {***REMOVED***))[ type ]) && cache[0] === dirruns ) {
							diff = cache[1];
						...
						***REMOVED*** else {
							...
							while ( (node = ++nodeIndex && node && node[ dir ] ||
								...
								if ( ( ofType ? node.nodeName.toLowerCase() === name : node.nodeType === 1 ) && ++diff ) {
									...
									if ( useCache ) {
										(node[ expando ] || (node[ expando ] = {***REMOVED***))[ type ] = [ dirruns, diff ];
									***REMOVED***
		...
		***REMOVED*** :
		...
		function( elem, context, xml ) {
			...
			***REMOVED*** else {
				while ( (elem = elem[ dir ]) ) {
					if ( elem.nodeType === 1 || checkNonElements ) {
						outerCache = elem[ expando ] || (elem[ expando ] = {***REMOVED***);
				...
				***REMOVED***
				return setMatcher(
					...
					i > 1 && toSelector(
						...
						tokens.slice( 0, i - 1 ).concat({ value: tokens[ i - 2 ].type === " " ? "*" : "" ***REMOVED***)
					).replace( rtrim, "$1" ),
					matcher,
...
function internalData( elem, name, data, pvt /* Internal Use Only */ ) {
	...
	***REMOVED***
	...
	if ( !cache[ id ] ) {
		...
		cache[ id ] = isNode ? {***REMOVED*** : { toJSON: jQuery.noop ***REMOVED***;
	***REMOVED***
...
function internalRemoveData( elem, name, pvt ) {
	...
	***REMOVED***
	...
	/* jshint eqeqeq: false */
	***REMOVED*** else if ( support.deleteExpando || cache != cache.window ) {
		/* jshint eqeqeq: true */
		delete cache[ id ];
...
(function() {
	...
	for ( i in { submit: true, change: true, focusin: true ***REMOVED***) {
		eventName = "on" + i;
		...
		if ( delegateCount && cur.nodeType && (!event.button || event.type !== "click") ) {
			...
			for ( ; cur != this; cur = cur.parentNode || this ) {
				...
				if ( cur.nodeType === 1 && (cur.disabled !== true || event.type !== "click") ) {
					...
					***REMOVED***
					if ( matches.length ) {
						handlerQueue.push({ elem: cur, handlers: matches ***REMOVED***);
					***REMOVED***
				***REMOVED***
			***REMOVED***
		***REMOVED***
		...
		if ( delegateCount < handlers.length ) {
			handlerQueue.push({ elem: this, handlers: handlers.slice( delegateCount ) ***REMOVED***);
		***REMOVED***
...
function fixCloneNodeIssues( src, dest ) {
	...
	***REMOVED*** else if ( nodeName === "object" ) {
		if ( dest.parentNode ) {
				...
				if ( data ) {
					...
					***REMOVED***
					...
					if ( cache[ id ] ) {
						...
						***REMOVED*** else if ( typeof elem.removeAttribute !== strundefined ) {
	...
	***REMOVED***);
	...
	function computeStyleTests() {
		...
		***REMOVED***
		...
		if ( window.getComputedStyle ) {
			pixelPositionVal = ( window.getComputedStyle( div, null ) || {***REMOVED*** ).top !== "1%";
			boxSizingReliableVal =
				( window.getComputedStyle( div, null ) || { width: "4px" ***REMOVED*** ).width === "4px";
			...
			reliableMarginRightVal =
				!parseFloat( ( window.getComputedStyle( contents, null ) || {***REMOVED*** ).marginRight );
		***REMOVED***
	...
	***REMOVED***
	...
	if ( !jQuery.isEmptyObject( orig ) ) {
		if ( dataShow ) {
			...
			***REMOVED***
		***REMOVED*** else {
			dataShow = jQuery._data( elem, "fxshow", {***REMOVED*** );
		***REMOVED***
...
boolHook = {
	set: function( elem, value, name ) {
		if ( value === false ) {
			...
			jQuery.removeAttr( elem, name );
		***REMOVED*** else if ( getSetInput && getSetAttribute || !ruseDefault.test( name ) ) {
	...
	toggleClass: function( value, stateVal ) {
		...
		***REMOVED***
		...
		return this.each(function() {
			...
			***REMOVED*** else if ( type === strundefined || type === "boolean" ) {
				if ( this.className ) {
...
var rvalidtokens = /(,)|(\[|{)|(***REMOVED***|])|"(?:[^"\\\r\n]|\\["\\\/bfnrt]|\\u[\da-fA-F]{4***REMOVED***)*"\s*:?|true|false|null|-?(?!0\d)\d+(?:\.\d+|)(?:[eE][+-]?\d+|)/g;
...
jQuery.fn.load = function( url, params, callback ) {
	...
	***REMOVED***
	...
	if ( self.length > 0 ) {
		...
		***REMOVED***).complete( callback && function( jqXHR, status ) {
