...
namespace PlataformaTransparencia.Negocios.Proyectos
{
    public class BllSearchFilters : RespuestaContratoBase
    {
        ...
        public void ObtenerFiltrosEspecificosParaProyectos(ModelDataFilters DataFilters)
        {
            ...
            var list5 = (from SectorFilters in DataModel.Sectors select new { name = SectorFilters.NombreSector, value = SectorFilters.IdSector }).ToList();
            if (list5.Count() > 0) {
            ...
            var list6 = (from OrgFinanciadorFilters in DataModel.CatalogoOrganismoFinanciadors select new { name = OrgFinanciadorFilters.OrganismoFinanciador, value = OrgFinanciadorFilters.IdOrganismoFinanciador }).ToList();
            if (list6.Count() > 0) {
