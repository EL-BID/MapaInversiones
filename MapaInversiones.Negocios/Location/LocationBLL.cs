...
namespace PlataformaTransparencia.Negocios.Location
{
    public class LocationBLL : ILocationBLL
    {
        ...
        {
            ModelLocationData objReturn = new ModelLocationData() { nomLocation = string.Empty, Encabezado = new Modelos.Location.InfoLocationGen { CantProyectos = 0, Costo = 0, Duracion = 0 } };
        ...
        private Modelos.Location.InfoLocationGen GetInfoLocation(string location_id, string type)
        {
            using (var DataModel = new TransparenciaDB())
            {
                var defaultRta = new Modelos.Location.InfoLocationGen { CantProyectos = 0, Costo = 0, Duracion = 0, IdDepartamento = string.Empty, IdMunicipio = string.Empty, urlImgXL = string.Empty };
                if (string.IsNullOrEmpty(type) || string.IsNullOrEmpty(location_id)) return defaultRta;
                if (type.ToUpper() == "REGION")
                {
