using System;
using System.Collections.Generic;
using System.Text;

namespace PlataformaTransparencia.Modelos
{
    public class Filter
    {
        /// <summary>
        /// Es el nombre del filtro
        /// </summary>
        public string name { get; set; ***REMOVED***

        /// <summary>
        /// Es el parametro del filtro
        /// </summary>
        public string parameter { get; set; ***REMOVED***

        /// <summary>
        /// Indica si el filtro permite multiple seleccion
        /// </summary>
        public bool esMultiple { get; set; ***REMOVED***

        /// <summary>
        /// Indica si el filtro permite multiple seleccion
        /// </summary>
        public bool usaServicioAjax { get; set; ***REMOVED***

        /// <summary>
        /// Indica si el filtro permite multiple seleccion
        /// </summary>
        public string urlServicioAjax { get; set; ***REMOVED***

        /// <summary>
        /// Indica la seccion del aplicativo donde se usa el filtro
        /// </summary>
        public string seccionAplicativo { get; set; ***REMOVED***

        /// <summary>
        /// Lista con los valores de los filtros.
        /// Tiene 2 tipos item, item geography.
        /// </summary>
        public List<itemFilters> items { get; set; ***REMOVED***

        public Filter()
        {
            items = new List<itemFilters>();
    ***REMOVED***

        public Filter(string nombreVisual, string nombreCodigo, string seccionFuncionalAplicativo)
        {
            name = nombreVisual;
            parameter = nombreCodigo;
            seccionAplicativo = seccionFuncionalAplicativo;
            items = new List<itemFilters>();
    ***REMOVED***





***REMOVED***
***REMOVED***
