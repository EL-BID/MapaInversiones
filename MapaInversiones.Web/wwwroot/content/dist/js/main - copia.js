						...
						response($.map(data, function (item) {
							...
							switch (item.Categoria) {
								...
								default:
									...
									$("#divResultados").append("<div class=\"search-item-t\"> <a class=\"enlace_resultados enlace_ficha\"  role=\"button\" data-parameter= '" + item.Id + "' data-type= '" + item.Categoria + "'\"> <img src=\"" + iconocategoria + "\" /> " + item.Nombre + "</a><div class=\"clearfix\"><a class=\"pull-left orangeLink enlace_proyecto\" role=\"button\" data-parameter= '" + item.Id + "' data-type= '" + item.Categoria + "'\">Ver Perfil <span class=\"glyphicon glyphicon-chevron-right\"</span></a></div></div>");
							}

							//$("#divResultados").append("<div class=\"search-item-t\"> <a class=\"enlace_resultados\" onclick=\"ocultarDiv('" + item.Nombre + "','" + item.Id + "');\"> <h2><img src=\"" + iconocategoria + "\" /> " + item.Nombre + "<a class=\"enlace_resultados\"> VER PERFIL \></a><a class=\"enlace_resultados\"> MOSTRAR EN EL MAPA \></a></h2></a></div>");
							//return true;
							//return {
							//    label: item.Nombre,
							//    value: item.Id,
		...
		initialize: function () {
			...
			router.configure({
				...
				on: function on() {
					...
					if (location.pathname.indexOf('contratistaprofile') !== -1 || location.pathname.indexOf('PerfilSector') !== -1) {
						if (self.firstTime) {
					...
					if (self.lastRequestedUrl !== location.hash) {
						...
						if (location.pathname.indexOf('LocationProfile') !== -1 || location.pathname.indexOf('PerfilSector') !== -1) {
							params = location.hash.match(/\?.+/)
							...
							if (params) {
								...
								for (var i = 0, pair; i < params.length; i++) {
									pair = params[i].split('=');
									if (pair[0] == 'departamento' || pair[0] == 'municipio' || pair[0] == 'sector') {
										paramName = pair[0];
										value = pair[1];
									}
								}
								if (paramName == 'departamento' || paramName == 'municipio' || paramName == 'sector') {
	...
	function (
		...
		function Map() {
			...
			function checkZoomChange() {
				...
				//console.log('1. ZOOOOOOOOOOOOOOOOOOM000000' + zoom)
		...
		var Filter = new Class(Observable, {
			initialize: function (data) {
				...
				if (!this.usaServicioAjax) {
					...
					//debugger;
					if (this.parameter.indexOf('estado') !== 0) tempArray.sort(this.alphabetSorting)
				}
	...
	function (Observable, Filter, appStates) {
		...
		var FiltersGroup = new Class(Observable, {
			...
			activateOptionsRelated: function (opts) {
				var filter, option, tmpFilterAct, tmpSecondFilterAct,
			...
			hideOtherFilters: function (filterActivated, cancelNotify) {
				...
				//debugger;
				if (filterActivated.parameter.indexOf('estado') !== 0) filterActivated.sortOptionsBy(activated)
...
define('location_profile', [
	...
	function (
		...
		if (location.pathname.indexOf('LocationProfile') !== -1) {
			...
			AgregarFiltros();
			comunes.load_filtro_orden("divFiltrosFichaOrdena", "filterByEjecucion", false);
			...
			loadProyectosEjecucion(proyectos_eje, "projects");
			configuraFiltrosEje();
		...
		if (location.pathname.indexOf('PerfilSector') !== -1) {
			...
			var searchProjectsList = '/api/serviciosproyectos/listadoBasico';
		...
		function inicializaDatos() {
			...
			var div_proyectos = d3.select("#Carousel");
			div_proyectos.remove();
			...
			if (imageURL != "") {
				$(".s0-section").css("background", "url('" + imageURL + "') no-repeat");
				$(".s0-section").css("color", "#FFF");
			} else {
				$(".s0-section").css("background", "url('" + "../../content/img/location/location-image.jpg" + "') no-repeat");
				$(".s0-section").css("color", "#FFF");
		...
		function loadConsolidaEstados() {
			...
			for (var i = 0; i < projectsPerEstado.length; i++) {
				if (i == 0) {
					txt_aux += "Hay " + projectsPerEstado[i].rawValue.toString() + " proyectos " + "en " + projectsPerEstado[i].label + ",";
				} else {
					if (i == projectsPerEstado.length - 1) {
						txt_aux += " y " + projectsPerEstado[i].rawValue.toString() + " en " + projectsPerEstado[i].label + ".";
					} else {
						txt_aux += " " + projectsPerEstado[i].rawValue.toString() + " en " + projectsPerEstado[i].label + ",";
					}
				}
		...
		function make_viz() {
			...
			var visualization = d3plus.viz()
				...
				.color({
					"scale": ["#2A6E80", "#A1ECFF", "#54DDFF", "#43B1CC", "#43B1CC"],
				})
				.legend(false)
				.resize(true)
				.draw()
		...
		function loadProyectosEjecucion(resultados, opc) {
		...
		function configuraFiltrosEje() {
			if ($("#filterByEjecucion").length > 0) {
				$('#filterByEjecucion li').bind('click onclick', function () {
					...
					var sorted = objJson;
					var val_Sel = $(this).attr("id").toUpperCase();
					...
					if (val_Sel != "") {
						...
						loadProyectosEjecucion(sorted, "filters");

					} else {
						//opcion vacia
						loadProyectosEjecucion(objFiltered, "filters");
					}
		...
		function AgregarFiltros_proy() {
			...
			$('#filterByEtapa li,#filterBySector li,#filterByDepartamento li,#filterEjecDepartamento li').bind('click onclick', function (e) {
		...
		function AgregarFiltros() {
			var filters = Services.filters.forProjects().done(function (result) {
				...
				for (var i = 0; i < items_result.length; i++) {
					if (items_result[i].parameter == "estado1") {
				...
				$('#filterEjecSector li').bind('click onclick', function () {
					...
					var opc_filtro = $("#filterByEjecucion").find("li.selected").attr("id");
					if (opc_filtro != undefined) {
						opc_filtro = opc_filtro.toString().toUpperCase();
					}
		...
		function cargarProyectos(pagina) {
			...
			var url = urlsMap.searchProjectsBasicList;
					...
					if (data.objects.length > 0) {
						...
						for (var k = 0; k < data.objects.length; k++) {
							...
							div_proy.append("a")
								...
								.attr("href", "../Proyecto/PerfilProyecto/" + data.objects[k].location)
		...
		function ordenaFechaIni(a, b) {
			var aa = a.FechaInicioProyecto.split('/').reverse().join(),
				bb = b.FechaInicioProyecto.split('/').reverse().join();
			return aa > bb ? 1 : -1;
...
require([
	...
	function (
		...
		AppState.on('Polygons-loaded', function () {
			...
			if ((location.pathname.indexOf('LocationProfile') !== -1 || location.pathname.indexOf('PerfilSector') !== -1) && AppState.filtersLoaded) {
				var locationID = document.body.getAttribute('data-locationID');
				var locationType = document.body.getAttribute('data-locationType');
		...
		FiltersManager.on('filters-loaded', function () {
			...
			if ((location.pathname.indexOf('LocationProfile') !== -1 || location.pathname.indexOf('PerfilSector') !== -1) && AppState.TerritoryLoaded) {
				var locationID = document.body.getAttribute('data-locationID');
				var locationType = document.body.getAttribute('data-locationType');
		...
		$("#divResultados").on('click', '.enlace_proyecto', function (e) {
			e.preventDefault()
			var dataValue = $(this).attr('data-parameter')
			var url = "/proyecto/perfilproyecto/" + dataValue
		...
		$("#divResultados").on('click', '.enlace_contratista', function (e) {
			...
			SearchViewModel.LoadContratProfile(dataType, dataValue, ruc);
			//var dataValue = $(this).attr('data-parameter')
			//var url = "/Sector/PerfilSector/" + dataValue
			//window.location.href = url
		});
		var SearchViewModel = {
			...
			LoadLocationProfile: function (dataType, dataValue) {
				...
				var url = "/localizacion/LocationProfile#/?" + dataType + "=" + dataValue;
				window.location.href = url;
			},
			...
			LoadContratProfile: function (dataType, dataValue, ruc) {
				document.cookie = "ruc=" + ruc + ";path=/;";
				//var url = "/contratista/contratistaprofile/?ruc=" + ruc;
