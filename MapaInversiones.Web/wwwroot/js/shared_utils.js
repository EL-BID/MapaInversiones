...
(function (global) {
    ...
    */
     ...
     */
    function formatoMoneda(value, decimales = 1, nivel = 0, tipoSufijo = 'long') {
        let escala = nivel;
        let val = value;

        // Auto-escalado: mientras val ≥ SALTO y haya sufijos
        while (escala < SINGULAR.length && val >= SALTO) {
            val /= SALTO;
            escala++;
        }

        // Si nivel=0 y no alcanzó millón, retorna sin sufijo
        if (nivel === 0 && escala === 0) {
        ...
        // Índice para sufijos
        const idx = escala - 1;
        // sufijo largo o corto  
        const sufijo = (tipoSufijo === 'long')
            ? (val === 1 ? SINGULAR[idx] : PLURAL[idx])
            : SHORT[idx];

        // Corregido: usar los separadores por defecto (coma decimal, punto miles)
