    ...
    data: {
      ...
      for (var i = 0; i < data.dataContratos.length; i++) {
          ...
          if (data.dataContratos[i].origenInformacion.toUpperCase().includes('ONCAE')) {
              if (existeOncae == 0) { $('#dataONCAE').empty(); existeOncae +=1;}
          ...
          if (data.dataContratos[i].origenInformacion.toUpperCase().includes('SEFIN')) {
              if (existeSefin == 0) { $('#dataSEFIN').empty(); existeSefin += 1; }
              var html = ''
      ...
      if (data.presupuestoInicial == null) { PresupuestoInicial.innerHTML = '- -';}
      else {
      ...
      if (data.presupuestoVigente == null) { PresupuestoVigente.innerHTML = '- -'; }
      else {
      ...
      if (data.presupuestoEjecutado == null) { PresupuestoEjecutado.innerHTML = '- -'; }
      else {
        ...
        success: function (result) {
            if (result.status == true) {
                if (result.cantidadTotalRegistros > 0) {
                    ...
                    if (scrol >= 1) {
                        $('html, body').animate({ scrollTop: $('#divSectionONCAE').offset().top }, 2000);
                    } else { scrol = scrol + 1; }
