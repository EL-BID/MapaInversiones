    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (items_result.length > 0) {
                GetProcesosCantidadEntidad(annio, entidad);
...
function GetProcesosCantidadEntidad(annio, entidad) {
    $.ajax({
        contentType: "application/json; charset=utf-8",
        url: "api/ServiciosContratos/GetCantidadProcesosEntidadesHomeContratos",
        type: "GET",
        data: {
            annio: annio,
            entidad: entidad,
        ...
        cantidadprocesos = data;
        
        GetProcesosEntidad(annio, entidad, 1 )
    }).fail(function (xhr, ajaxOptions, thrownError) {
        alert("Error " + xhr.status + "_" + thrownError);
    });

}
function GetProcesosEntidad(annio, entidad, pagina) {
    $("#divListadoInstituciones").html("");
    $("#divPagFichas").html("");
    $("#divListadoInstituciones").html("<div id='search-loading'><p><i class='fa fa-spinner fa-spin'></i> Buscando...</p></div>");

    const key = annio + "_" + entidad + "_" + pagina;

    if (cacheProcesos[key]) {

        GetListadoProcesos(cacheProcesos[key]);
        dibujarPagNumeradas(pagina);
        ...
        $.ajax({
            ...
            data: {
                ...
                entidad: entidad,
                numeroPagina: pagina,
                registrosPorPagina: cantXPagina
            }
        }).done(function (data) {

            cacheProcesos[key] = data.unidadCompras;
...
function dibujarPagNumeradas(paginaActual) {
    var totalNumber = cantidadprocesos;
    ...
    //var pagesHTML = '';
    //var cant_por_pag = 6;
    var totalNumerosPaginador = cantXPagina;
    $("#divPagFichas").html("");
    ...
    $('#page_right, #page_left, .page_left, .page_right')
        .off('click')        // 👈 evita duplicados
        .on('click', function () {
            paginaActual = $(this).attr("data-page");
            $("#divListadoEntidad").empty();

            GetProcesosEntidad(
                $("#top_contratos_periodos option:selected").val(),
                $('#sltEntidad option:selected').val(),
                paginaActual
            );
        });
...
$("#sltEntidad").change(function () {
    GetProcesosCantidadEntidad($("#top_contratos_periodos option:selected").val(), $("#sltEntidad option:selected").val());
