					...
					if ( parent ) {
						...
						***REMOVED***
						...
						if ( forward && useCache ) {
							...
							node = parent;
							outerCache = node[ expando ] || ( node[ expando ] = {***REMOVED*** );

							// Support: IE <9 only
							// Defend against cloned attroperties (jQuery gh-1709)
							uniqueCache = outerCache[ node.uniqueID ] ||
								( outerCache[ node.uniqueID ] = {***REMOVED*** );
				...
				while ( ( elem = elem[ dir ] ) ) {
					if ( elem.nodeType === 1 || checkNonElements ) {
						...
						uniqueCache = outerCache[ elem.uniqueID ] ||
							( outerCache[ elem.uniqueID ] = {***REMOVED*** );
			...
			if ( matcher[ expando ] ) {
				...
				***REMOVED***
				return setMatcher(
					...
					tokens
						.slice( 0, i - 1 )
						.concat( { value: tokens[ i - 2 ].type === " " ? "*" : "" ***REMOVED*** )
					).replace( rtrim, "$1" ),
					matcher,
		...
		while ( ( matched = handlerQueue[ i++ ] ) && !event.isPropagationStopped() ) {
			...
			while ( ( handleObj = matched.handlers[ j++ ] ) &&
				...
				if ( !event.rnamespace || handleObj.namespace === false ||
					...
					ret = ( ( jQuery.event.special[ handleObj.origType ] || {***REMOVED*** ).handle ||
	...
	jQuery.event.add( el, type, {
		...
		handler: function( event ) {
			...
			if ( ( event.isTrigger & 1 ) && this[ type ] ) {
				...
				***REMOVED*** else if ( ( jQuery.event.special[ type ] || {***REMOVED*** ).delegateType ) {
					event.stopPropagation();
				***REMOVED***
		...
		function done( status, nativeStatusText, responses, headers ) {
			...
			***REMOVED***
			...
			if ( !isSuccess && jQuery.inArray( "script", s.dataTypes ) > -1 ) {
				s.converters[ "text script" ] = function() {***REMOVED***;
			***REMOVED***
