...
# Definir vector_settings correctamente como una instancia de VectorSettings
...
class VectorStore:
    ...
    def __init__(self):
        ...
        self.vec_client = client.Sync(
            ...
            vector_settings.embedding_dimensions,
            time_partition_interval=vector_settings.time_partition_interval,
        )

        # sql server connection from settings parameters:
        self.sqlserverconn = psycopg.connect(settings.psycopg_conn_string)
    ...
    def keyword_search(
        ...
        search_sql = f"""
        SELECT id, contents, ts_rank_cd(to_tsvector('spanish', contents), query) as rank
        FROM {self.vector_settings.table_name}, websearch_to_tsquery('spanish', %s) query
        WHERE to_tsvector('spanish', contents) @@ query
        ORDER BY rank DESC
        LIMIT %s
        """
    ...
    def _rerank_results(
        ...
        """
        Re-rank the combined search results using FlashRank and return a sorted DataFrame.

        Args:
            query: The search query string.
            combined_results: A DataFrame containing combined search results.
            top_n: The number of top results to return.

        Returns:
            A sorted DataFrame with columns: id, content, search_type, relevance_score.
        """
        try:
            ...
            logger.error(
                "FlashRank is not installed. Install with: pip install flashrank"
            )
            logger.warning("Returning unranked results")
            return combined_results

        # Inicializa el cliente de FlashRank
        flashrank_client = Ranker(max_length=128, cache_dir="/app/.cache/flashrank")

        # Crea una instancia de FlashrankRerank
        reranker = FlashrankRerank(client=flashrank_client, top_n=top_n)

        # Prepara los documentos para el reranking
        documents = [
            Document(
                page_content=row["content"],
                metadata={"id": row["id"], "search_type": row["search_type"]},
            )
        ...
        # Construye un DataFrame con los resultados rerankeados
            ...
            [
                {
                    ...
                    "relevance_score": doc.metadata[
                        "relevance_score"
                    ],  # Cambiado para usar 'relevance_score'
                }
                for doc in reranked_docs
            ]
        )

        # Ordena los resultados por la puntuación de relevancia en orden descendente
        reranked_df = reranked_df.sort_values("relevance_score", ascending=False)
